...
class PromptFile:
    ...
    default_file_path = os.path.join(
        os.path.dirname(__file__), "..", default_context_filename
    )
    default_config_path = os.path.join(
        os.path.dirname(__file__), "..", "current_context.config"
    )
    ...
    def add_input_output_pair(self, user_query, prompt_response):
        ...
        if self.config["multi_turn"] == "on":
            self.config["token_count"] += len(user_query.split()) + len(
                prompt_response.split()
            )
            self.set_config(self.config)
    ...
    def clear(self):
        ...
        with open(self.file_path, "r") as f:
            ...
            filename = os.path.join(
                os.path.dirname(__file__), "..", "deleted", filename
            )
    ...
    def load_context(self, filename, initialize=False):
        ...
        filepath = Path(
            os.path.join(os.path.dirname(__file__), "..", "contexts", filename)
        )
